...
#~ libgl1-mesa-dev libegl1-mesa-dev libegl1-mesa libgles2-mesa-dev ; then
...
cd ~/Downloads
#~ if wget https://gstreamer.freedesktop.org/src/gst-plugins-bad/gst-plugins-bad-1.14.4.tar.xz ; then
	#~ printf '\nDownloaded Gst-Plugins-Bad\n\n'
#~ else
	#~ printf '\nFailed to download gst-plugins-bad\n\n'
	#~ rm gst-plugins-bad-1.14.4.tar.xz
	#~ exit 1
#~ fi

#~ tar xf gst-plugins-bad-1.14.4.tar.xz
cd gst-plugins-bad-1.14.4

printf '########################\n\n'
printf '\nInstalling Gst-Plugins-Bad for Pi\n\n'
...
#~ if sudo checkinstall --install=no ; then
	#~ printf '\nGenerated .deb File Successfully.\n\n'
#~ else
	#~ printf '\nFailed to Generate .deb File.\n\n'
	#~ exit 1
#~ fi

#~ if sudo dpkg -i --force-overwrite gst-plugins-bad_1.10.4-1_armhf.deb ; then
	#~ printf 'Installed Gst-Plugins-Bad-1.10.4 Successfully\n\n'
#~ else
	#~ printf 'Failed to Install Gst-Plugins-Bad .deb File\n\n'
	#~ exit 1
#~ fi
...
#~ printf 'Now Installing GST-OMX\n\n'

#~ printf 'Downloading Custom GST-OMX .deb File\n\n'
#~ if wget http://steinerdatenbank.de/software/gstreamer1.0-omx-rpi_1.10.5-1+rpi+patches_armhf.deb ; then
	#~ printf '\nDownloaded Successfully\n\n'
#~ else
	#~ printf '\nFailed to Download GST-OMX\n\n'
	#~ exit 1
#~ fi


#~ printf 'Now Installing GST-OMX .deb to the System\n\n'
#~ if sudo dpkg -i gstreamer1.0-omx-rpi_1.10.5-1+rpi+patches_armhf.deb ; then
	#~ printf '\nInstalled Everything Succesfully!\n\n'
#~ else
	#~ printf '\nFailed to Install GST-OMX .deb File\n\n'
	#~ exit 1
#~ fi
